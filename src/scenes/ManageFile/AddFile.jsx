...
const AddFile = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3001/read/createFile`,
        formData,
        {
          headers: {
  ...
  const selectSubDepartment = (
    ...
    handleChange2,
    customSubDepartment,
    handleCustomInputChange2
  ) => {
    const filteredBooks = books.filter(
      (book) => book.department === selectedDepartment
    );
    const uniqueSubDepartments = [
      ...new Set(filteredBooks.map((book) => book.subDepartment)),
    ];

    return (
      <FormControl sx={{ m: 1, width: "60%" }}>
        <InputLabel id="select-label-subDepartment">
  ...
  return (
    ...
    >
      ...
      {selectDepartment(
        ...
        customDepartment,
        (e) => setCustomDepartment(e.target.value)
      )}
      ...
      {selectedDepartment &&
        selectSubDepartment(
          ...
          customSubDepartment,
          (e) => setCustomSubDepartment(e.target.value)
        )}
      <TextField
        sx={{ width: "60%" }}
        required
      ...
      />
      <Button sx={{ width: "60%" }} variant="contained" onClick={handleSubmit}>
        Submit
      </Button>
    </Stack>
  );
